# "Mock" in the name is a misnomer — this hits a real PostgreSQL database.
# It's called mock because it's the default dev implementation, not a real production backend.
            ...
            with conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    cur.execute(
                        ...
                        RETURNING id, correlation_token, sent_at, deadline_at
                        """,
                        (
                            tenant_id,
        ...
        try:
            with conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    cur.execute(
                        """
                        SELECT pa.id, pa.workflow_instance_id, pa.status,
